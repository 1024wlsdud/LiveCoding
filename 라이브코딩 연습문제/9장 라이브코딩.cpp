...
public:
    StudentWorker(string varName, string name, int id, string university, int year, int tuition,
         string company, int payPerHour, int overtime);
    ~StudentWorker();
...
StudentWorker::StudentWorker(string varName, string name, int id, string university, int year, int tuition,
    string company, int payPerHour, int overtime) : Person(varName, name, id), 
    Student(varName, name, id, university, year, tuition),
    Employee(varName, name, id, company, payPerHour, overtime) {
    cout << "    StudentWorker()" << endl;
}
StudentWorker::~StudentWorker() {
    cout << varName << ": ~StudentWorker()" << endl;
}
...
void StudentWorker::whatAreYouDoing() {
    cout << varName << ": StudentWorker::whatAreYouDoing(): I am studying and working." << endl;
}
int StudentWorker::operator () () {
    int total = Student::operator()() + Employee::operator()();
    if (total > year * tuition * 2)
        total = year * tuition * 2;
    cout << varName << ": StudentWorker::operator(): 장학금과 아르바이트비 합계: " << total << endl;
    return total;
}
int main()
{
    StudentWorker sw("sw", "성춘향", 10003,
        "IT University", 3, 3000000,
        "GS25", 8800, 15);
    cout << endl;
