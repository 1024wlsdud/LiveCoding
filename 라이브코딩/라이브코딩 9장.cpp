...
void Shape::move(int width, int height) {
...
public:
    ...
    virtual void move(int width, int height)override;
};
...
void Circle::draw() { // 반지름은 to_string(radius)를 이용하여 문자열로 변화
   ...
   //  화면에 반지름과 중심 좌표 값을 출력, 예) "Circle 5 (20,30)"
    cout << "Circle " << to_string(radius) << " " << center.toString() << endl;
}
